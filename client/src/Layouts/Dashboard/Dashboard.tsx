...
function Dashboard() {
  ...
  function calcMargin() {
    ...
    const sideMargin = isMobileNav ? "1em" : `${parseInt(navWidth) + 28}px`;
    return `2em ${sideMargin} 0 ${sideMargin}`;
  }

  return (
    <Box sx={{ ...styles.container, margin: calcMargin() }}>
      <DashboardTitle iconSrc={VegetationIndicesIcon}  text={"Vegetation Indices"} />
      <DashboardGraphs />
    </Box>
  );
}
